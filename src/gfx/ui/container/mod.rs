...
pub trait Container: UIElement {
    ...
    fn child_horizontal(&self) -> Layout { self.container_data().child_horizontal }
    fn child_horizontal_mut(&mut self) -> &mut Layout { &mut self.container_data_mut().child_horizontal }
    fn child_vertical(&self) -> Layout { self.container_data().child_vertical }
    fn child_vertical_mut(&mut self) -> &mut Layout { &mut self.container_data_mut().child_vertical }

    fn spacing(&self) -> u32 { self.container_data().spacing }
    fn spacing_mut(&mut self) -> &mut u32 { &mut self.container_data_mut().spacing }
}